...
namespace Futureverse.UBF.Runtime
{
	...
	/// <summary>
	///     Unity exposes the PlayerLoop to allow you to insert your own "systems" to be run in similar ways to e.g. Update or
	///     FixedUpdate.
	///     The interface for that is a bit hairy, and there are bugs that needs workarounds, so this is a nice interface for
	///     interacting with that system.
	///     In essence, use PlayerLoopInterface.InsertSystemBefore/After to have a callback be executed every frame, before or
	///     after some built-in system.
	///     The built-in systems live in UnityEngine.Experimental.PlayerLoop, so if you want to insert a system to run just
	///     before Update, call:
	///     PlayerLoopInterface.InsertSystemBefore(typeof(MyType), MyMethod, typeof(UnityEngine.PlayerLoop.Update);
	///     If you want to run in the fixed time step (FixedUpdate), you have to insert the system as a subsystem of
	///     UnityEngine.PlayerLoop.FixedUpdate. For example, use
	///     UnityEngine.PlayerLoop.FixedUpdate.ScriptRunBehaviourFixedUpdate:
	///     PlayerLoopInterface.InsertSystemBefore(typeof(MyType), MyMethod,
	///     typeof(UnityEngine.PlayerLoop.FixedUpdate.ScriptRunBehaviourFixedUpdate);
	/// </summary>
	internal static class PlayerLoopInterface
	{
		private static readonly List<PlayerLoopSystem> s_insertedSystems = new();
		...
		{
			foreach (var playerLoopSystem in s_insertedSystems)
			{
				TryRemoveSystem(playerLoopSystem.type);
			}

			s_insertedSystems.Clear();
		...
		public static void InsertSystemAfter(
			Type newSystemMarker,
			PlayerLoopSystem.UpdateFunction newSystemUpdate,
			Type insertAfter)
		{
			var playerLoopSystem = new PlayerLoopSystem
			{
				type = newSystemMarker,
				updateDelegate = newSystemUpdate,
			};
		...
		public static void InsertSystemBefore(
			Type newSystemMarker,
			PlayerLoopSystem.UpdateFunction newSystemUpdate,
			Type insertBefore)
		{
		...
		/// <param name="insertAfter">The subsystem to insert the system after</param>
		private static void InsertSystemAfter(PlayerLoopSystem toInsert, Type insertAfter)
		{
			if (toInsert.type == null)
			{
				throw new ArgumentException(
					"The inserted player loop system must have a marker type!",
					nameof(toInsert.type)
				);
			}
			...
			{
				throw new ArgumentException(
					"The inserted player loop system must have an update delegate!",
					nameof(toInsert.updateDelegate)
				);
			}
			...
			{
				throw new ArgumentException(
					$"When trying to insert the type {toInsert.type.Name} into the player loop after {insertAfter.Name}, " +
					$"{insertAfter.Name} could not be found in the current player loop!"
				);
			}

			s_insertedSystems.Add(toInsert);
		...
		/// <param name="toInsert">System to insert. Needs to have updateDelegate and Type set.</param>
		/// <param name="insertBefore">The subsystem to insert the system before</param>
		private static void InsertSystemBefore(PlayerLoopSystem toInsert, Type insertBefore)
		{
			if (toInsert.type == null)
			{
				throw new ArgumentException(
			...
			{
				throw new ArgumentException(
					$"When trying to insert the type {toInsert.type.Name} into the player loop before {insertBefore.Name}, " +
		...
		/// <summary>
		///     Tries to remove a system from the PlayerLoop. The first system found that has the same type identifier as the
		///     supplied one will be removed.
		/// </summary>
		/// <param name="type">Type identifier of the system to remove</param>
		/// <returns></returns>
		private static bool TryRemoveSystem(Type type)
		{
			if (type == null)
			{
		...
		public static PlayerLoopSystem CopySystem(PlayerLoopSystem system)
		{
			...
			if (system.subSystemList != null)
			{
				...
				for (var i = 0; i < copy.subSystemList.Length; i++)
				{
		...
		public static string CurrentLoopToString()
			=> PrintSystemToString(PlayerLoop.GetCurrentPlayerLoop());
