...
namespace Futureverse.UBF.UBFExecutionController.Samples
{
	public class AssetUI : MonoBehaviour
	{
		...
		public void Load(Asset asset, Action onClick)
		{
			...
			if (TryGetProfileImageUrl(asset, out var imageUrl))
			{
				StartCoroutine(LoadBackgroundImage(asset.TokenId, imageUrl));
			}
		}
		
		private static bool TryGetProfileImageUrl(Asset asset, out string url)
		{
			url = asset?.Metadata?.Properties?["image"]
				?.ToString();
			return url != null;
		}
